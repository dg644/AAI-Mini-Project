...
def random_oversample(image_dir, target_num_images):
    ...
    Usecase: After duplicating the data x4.75, there is still not enough pain data, so we need to randomly duplicate some pain images to get 24007
    """
...
def pain_generate_smote(train_pain_dir, synthetic_train_pain_dir):
    ...
    for person_id, pain_images in batched_people_pain_images.items():
        ...
        original_images_set = set(map(tuple, pain_numpy_arrays))
        #get only the synthetic images that are not in the original pain_numpy_arrays
        synthetic_pain_images = np.array([img for img in smote_pain_images if tuple(img) not in original_images_set])
